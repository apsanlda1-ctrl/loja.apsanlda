/* APSAN — correção definitiva do botão de publicação física.
   Não altera nem oculta campos. Assume o envio do formulário físico,
   guarda a imagem e publica imediatamente no marketplace.
*/
(function(){
'use strict';
...
async function publish(e){
  ...
  try{
    ...
    const product={
      ...
      publishedAt:new Date().toISOString(),status:'approved',approved:true,approvedAt:new Date().toISOString(),uploadStatus:'ready',uploadProgress:100,rejectionReason:'',deleted:false,salesCount:0,totalSold:0,
    ...
    form.reset();
    if(typeof window.resetType==='function')window.resetType();
...
function bind(){
  const form=document.getElementById('apsanPhysicalInnerForm');
  if(!form||form.dataset.apsanPublishFixed==='1')return;
  form.dataset.apsanPublishFixed='1';
