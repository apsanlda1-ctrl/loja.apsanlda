/* APSAN — correção segura do Mercado Nacional v2
   1) Mantém apenas um botão Mercado Nacional na landing page.
   2) Alimenta o catálogo diretamente de apsan_produtos/localStorage.
      Assim, produtos aprovados pelo administrador passam a aparecer.
...
(function(){
  ...
  function approvedProducts(){
    try{
      const list = typeof getData === 'function' && typeof PRODUCTS_KEY !== 'undefined'
        ? getData(PRODUCTS_KEY)
        : JSON.parse(localStorage.getItem('apsan_produtos') || '[]');
      return (Array.isArray(list) ? list : []).filter(p => p && p.status === 'approved' && !p.deleted && p.hidden !== true && p.isPublished !== false);
    }catch(e){
  ...
  function typeOf(p){
    const t=String(p.productType||p.type||p.kind||p.format||p.contentType||'').toLowerCase();
  ...
  function renderCards(){
    ...
    document.querySelectorAll('[data-mna-fix-product]').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  function dedupeHeroButton(){
    ...
    if(!hero)return;
    const matches=Array.from(hero.querySelectorAll('button,a')).filter(el=>/Mercado Nacional de Angola/i.test(el.textContent||''));
    if(matches.length>1){
      matches.slice(1).forEach(el=>el.remove());
    }
    if(matches.length===0){
      const b=document.createElement('button');
      b.type='button'; b.className='btn-about';
      b.innerHTML='<i class="fa-solid fa-store"></i> Mercado Nacional de Angola';
      b.addEventListener('click',()=>window.openMercadoNacionalAngola&&window.openMercadoNacionalAngola());
      hero.appendChild(b);
    }
  }
  ...
  function wire(){
    ...
    if(typeof open==='function' && !open.__apsanFixWrapped){
      ...
      wrapped.__apsanFixWrapped=true;
      window.openMercadoNacionalAngola=wrapped;
    }
    ...
    setInterval(()=>{ if(document.getElementById('mercadoNacionalAngola')?.classList.contains('mna-open')) renderCards(); },3000);
  }
